...
<head>
  ...
  <style>
    ...
    #final-card{display:block;margin:30px auto;max-width:650px;border-radius:25px;overflow:hidden;box-shadow:0 25px 60px rgba(0,0,0,0.4)}
  </style>
</head>
<body>
  ...
  <script>
    ...
    function choose(design) {
      fetch("/api/set-design", {method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({sessionId:sid,design})});
          ...
          if (d.success) {
            ...
            document.getElementById("status").innerHTML = 
              ...
              '<script>' +
                ...
                'const img=new Image();' +
                'img.onload=()=>{ctx.drawImage(img,0,0,950,600);' +
                  // Имя держателя — слева внизу
                  'ctx.font="bold 48px Arial";ctx.fillStyle="#ffffff";ctx.strokeStyle="#000000";ctx.lineWidth=6;' +
                  'ctx.strokeText("'+name+'", 130, 480);ctx.fillText("'+name+'", 130, 480);' +
                  // Номер карты — по центру снизу
...
app.listen(PORT, "0.0.0.0", () => console.log(`Запущен на порту ${PORT}`));